...
public class SmartFontGenerator {
	...
	public BitmapFont createFont(FileHandle fontFile, String fontName, int fontSize, int shadowX, int shadowY, float borderWidth) {
		BitmapFont font = null;
		// if fonts are already generated, just load from file
		Preferences fontPrefs = Gdx.app.getPreferences("com.unshape.game");
		...
		if (!loaded || forceGeneration) {
			...
			font = generateFontWriteFiles(fontName, fontFile, fontSize, pageSize, pageSize, shadowX, shadowY, borderWidth);
		}
		return font;
	}
	 ...
	 * @param fontSize
	 */
	private BitmapFont generateFontWriteFiles(String fontName, FileHandle fontFile, int fontSize, int pageWidth, int pageHeight, int shadowX, int shadowY, float borderWidth) {
		FreeTypeFontGenerator generator = new FreeTypeFontGenerator(fontFile);
		FreeTypeFontParameter parameter = new FreeTypeFontParameter();
		PixmapPacker packer = new PixmapPacker(pageWidth, pageHeight, Pixmap.Format.RGBA8888, 2, false);
		
		parameter.size = fontSize;
		parameter.shadowColor = new Color(0.1f, 0.1f, 0.1f, 0.5f);
